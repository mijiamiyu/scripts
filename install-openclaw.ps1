...
# 清掉学员机器可能残留的代理 env(VPN/翻墙软件关掉后留的),避免 npm 走死代理连不上
$env:HTTP_PROXY = $null
...
function Step-Onboard {
    ...
    try {
        ...
        $scriptText = $webClient.DownloadString($scriptUrl)
        Invoke-Expression $scriptText
        return $true
    } catch {
        Write-Warn "加载模型配置脚本失败: $_"
        Write-Info "回退到 openclaw onboard 交互配置"
